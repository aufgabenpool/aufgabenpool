...
(async () => {
    ...
    for (
        ...
        i < exercises.length && i < last_exercise_id;
        i++
    ) {
        ...
        if (
            fs.existsSync(old_content_path + question_id + '.xml') &&
            ...
            let xml_old = fs.readFileSync(
                old_content_path + question_id + '.xml',
                'utf8',
            );
            let xml_new = fs.readFileSync(
                img_out_path + question_id + '.xml',
                'utf8',
            );
        ...
        if (must_update == false) {
            ...
            for (let k = 0; k < 3; k++) {
                fs.copyFileSync(
                    old_content_path + question_id + '_' + k + '.png',
