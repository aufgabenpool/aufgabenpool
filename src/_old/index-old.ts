...
const MOODLE_EDIT_QUESTION_PATH =
    'https://aufgabenpool.th-koeln.de/moodle/question/question.php?&courseid=2&id=';
...
export function setImage(moodleID: number, previewIdx: number) {
    ...
    (<HTMLImageElement>imgElement).src =
        'data/' + moodleID + '_' + previewIdx + '.png';
    for (let i = 0; i < 4; i++) {
        let btnElement = document.getElementById(
            'preview_btn' + i + '_' + moodleID,
        );
        if (i == previewIdx) btnElement.classList.add('active');
...
function create_tax_head(id: string, color = 'dark', link = '') {
    return (
        '<br/><button id="tax_head_' +
        id +
        '" type="button" class="btn btn-' +
        color +
        ' btn-sm m-1 py-0" onclick="window.open(\'' +
        link +
        "', '_blank');\">" +
        id +
        '</button>'
...
function create_tax_element(id: string, text: string, description = '') {
    ...
    tax_texts[id] = text;
    tax_cnt[id] = cnt;
    if (!(id in tax_selected)) tax_selected[id] = false;
    let html_cnt =
        cnt == 0
            ? ''
    ...
    let html =
        '<button id="tax_element_' +
        id +
        '" type="button" class="btn btn-outline-dark btn-sm py-0 my-1" onclick="aufgabenpool.clicked_tax_element(\'' +
        id +
        '\');"' +
        desc_props +
        '>' +
...
function mark_tag_element(id: string) {
    ...
    if (tax_cnt[id] > 0)
        element.innerHTML =
            ...
            '</span></button>' +
            '</b>';
    else element.innerHTML = '<b>' + text.toUpperCase() + '</b>';
}
...
function build_document() {
    ...
    for (let i = 0; i < metadata['taxonomy'].length; i++) {
        ...
        if (tokens[0] === 'head') {
            ...
            let color = tokens[2];
            let url =
                tokens[3] in metadata['taxonomy_urls']
                    ? metadata['taxonomy_urls'][tokens[3]]
                    : '';
            tags_html += i == 0 ? '' : '<br/>';
            ...
            if (name.length > 0)
                tags_html +=
                    '<br/>' + create_tax_desc(id, currentHeadColor, name);
            else tags_html += '<br/>&nbsp;';
...
function build_topic_hierarchy() {
    ...
    for (let te1_i in metadata['topic_hierarchy']) {
        ...
        if (tax_selected[te1_i]) mark_tag_element(te1_i);
    }
    tediv.innerHTML += '<br/>';
    // - second level
        ...
        for (let te1_i in metadata['topic_hierarchy']) {
            if (selected_te1.includes(te1_i)) {
                for (let te2_j in metadata['topic_hierarchy'][te1_i]) {
                    ...
                    if (tax_selected[te2_j]) mark_tag_element(te2_j);
                }
            }
        }
    }
            ...
            if (selected_te1.includes(te1_i)) {
                for (let te2_j in metadata['topic_hierarchy'][te1_i]) {
                    if (selected_te2.includes(te2_j)) {
                        for (let te3_k in metadata['topic_hierarchy'][te1_i][
                            te2_j
                        ]) {
                            let tokens = te3_k.split('_');
                            let topic = tokens[tokens.length - 1];
                            tediv.innerHTML += create_tax_element(te3_k, topic);
                            if (tax_selected[te3_k]) mark_tag_element(te3_k);
                        }
                    }
                }
            }
...
function build_exercise(idx: number, exercise: any, isBasket = false) {
    ...
    if (title.startsWith('0'))
        // TODO: remove this as soon as numbers are removed from Moodle
        title = title.substr(5);
    let optional_buttons = '';
    if (isBasket) optional_buttons += templates.remove_exercise_template;
    else optional_buttons += templates.add_exercise_to_basket_template;
    exercise_html = exercise_html.replaceAll(
        '!OPTIONAL_BUTTONS!',
        optional_buttons,
    );
    ...
    if (exercise_type === 'stack') exercise_type = 'STACK';
    else if (exercise_type === 'multichoice') exercise_type = 'MULTIPLE CHOICE';
    else if (exercise_type === 'truefalse') exercise_type = 'WAHR/FALSCH';
    ...
    for (let j = 0; j < exercise_tags_sorted.length; j++) {
        ...
        if (tag.startsWith('te_1_'))
            exercise_topic_html +=
                '<span class="">' +
                capitalize_each_word(tag.substr(5)) +
                '</span>&nbsp;';
    }
    // te_2
    for (let j = 0; j < exercise_tags_sorted.length; j++) {
    ...
    for (let j = 0; j < exercise_tags_sorted.length; j++) {
        ...
        if (tag.startsWith('bloom_'))
            exercise_tags_html +=
                '<span class="badge rounded-pill" style="background-color:#e85b22;">' +
                tag_name +
                '</span>&nbsp;';
    }
    // maier
    for (let j = 0; j < exercise_tags_sorted.length; j++) {
        let tag = exercise_tags_sorted[j];
        let tag_name =
            taxonomy_tag_dim_names[tag] + ': ' + taxonomy_tag_names[tag];
        if (tag.startsWith('maier_'))
            exercise_tags_html +=
                '<span class="badge rounded-pill" style="background-color:#b42b83;">' +
                tag_name +
                '</span>&nbsp;';
    }
    // TODO: praxiserprobt!
...
function build_exercises_tree() {
    ...
    for (let i = 0; i < metadata_exercises.length; i++) {
        ...
        if (selected_te1.length == 0) continue;
        if (arr_contains(selected_te1, exercise_tags) == false) continue;
        // filter te_2_
        let selected_te2 = get_tagdim_selection('te_2_');
        if (
            selected_te2.length > 0 &&
            arr_contains(selected_te2, exercise_tags) == false
        )
            continue;
        // filter te_3_
        let selected_te3 = get_tagdim_selection('te_3_');
        if (
            selected_te3.length > 0 &&
            arr_contains(selected_te3, exercise_tags) == false
        )
            continue;
        ...
        if (
            selected_bloom.length > 0 &&
            arr_contains(selected_bloom, exercise_tags) == false
        )
            continue;
        let selected_maier_1 = get_tagdim_selection('maier_1_');
        if (
            selected_maier_1.length > 0 &&
            arr_contains(selected_maier_1, exercise_tags) == false
        )
            continue;
        let selected_maier_2 = get_tagdim_selection('maier_2_');
        if (
            selected_maier_2.length > 0 &&
            arr_contains(selected_maier_2, exercise_tags) == false
        )
            continue;
        let selected_maier_3 = get_tagdim_selection('maier_3_');
        if (
            selected_maier_3.length > 0 &&
            arr_contains(selected_maier_3, exercise_tags) == false
        )
            continue;
        let selected_maier_4 = get_tagdim_selection('maier_4_');
        if (
            selected_maier_4.length > 0 &&
            arr_contains(selected_maier_4, exercise_tags) == false
        )
            continue;
        let selected_maier_5 = get_tagdim_selection('maier_5_');
        if (
            selected_maier_5.length > 0 &&
            arr_contains(selected_maier_5, exercise_tags) == false
        )
            continue;
        let selected_maier_6 = get_tagdim_selection('maier_6_');
        if (
            selected_maier_6.length > 0 &&
            arr_contains(selected_maier_6, exercise_tags) == false
        )
            continue;
        let selected_maier_7 = get_tagdim_selection('maier_7_');
        if (
            selected_maier_7.length > 0 &&
            arr_contains(selected_maier_7, exercise_tags) == false
        )
            continue;
        let selected_type = get_tagdim_selection('type_');
        if (
            selected_type.length > 0 &&
            arr_contains(selected_type, exercise_tags) == false
        )
            continue;
...
function build_basket_tree() {
    ...
    if (basket.length == 0) {
        ...
        exercises_html +=
            '<p class="p-0"><button type="button" class="btn btn-outline-danger btn-lg" onclick="aufgabenpool.download_basket();" data-toggle="tooltip" data-placement="top" title="Aufgabenblatt im Format Moodle-XML herunterladen"><i class="fa fa-download" aria-hidden="true"></i></button></p>';
    }
...
export function toggleTag(tag_idx: number) {
    ...
    if (buttonData.selected)
        tagElement.setAttribute(
            'class',
            'btn btn-' + buttonData.color + ' btn-sm m-1',
        );
    else
        tagElement.setAttribute(
            'class',
            'btn btn-outline-' + buttonData.color + ' btn-sm m-1',
        );
    build_exercises_tree();
}
...
export function download_basket() {
    basket_xml = '';
    if (basket.length > 0) download_basket_next(0);
}
...
function download_basket_next(basket_idx: number) {
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            ...
            basket_idx++;
            if (basket_idx < basket.length) download_basket_next(basket_idx);
            else download_basket_final();
        },
        error: function (xhr, status, error) {
            alert('ERROR: ' + xhr.responseText);
        },
